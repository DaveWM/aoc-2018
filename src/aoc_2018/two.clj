...
(defn contains-repeated-letter? [s n]
  (->> s
       frequencies
       (some (fn [[_ c]] (= c n)))))

(defn checksum [xs]
  (let [strings-with-2-repeats (filter #(contains-repeated-letter? % 2) xs)
        strings-with-3-repeats (filter #(contains-repeated-letter? % 3) xs)]
    (* (count strings-with-2-repeats) (count strings-with-3-repeats))))

(defn different-by-one-letter [xs]
  (let [str-len (count (first xs))]
    (->> (for [s1 xs
               s2 xs]
           (when (= (->> (map = s1 s2)
                         (filter identity)
...
(comment
  (checksum inputs)
  ; 5976
  (->> (different-by-one-letter inputs)
       same-letters)
  )